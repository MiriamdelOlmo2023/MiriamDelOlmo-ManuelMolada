...
document.getElementById("borrarApuesta").addEventListener("click", function() {
    ...
    if(numero !== ""){
        var numeroFichas = parseInt(document.getElementById("fichas").innerHTML);
        document.getElementById("fichas").innerHTML = (numeroFichas + parseInt(numero));
        document.getElementById("fichas1").innerHTML = (numeroFichas + parseInt(numero));
        llamada(parseInt(numero), numeroFichas);
...
document.addEventListener("generarTiradaTrue", function() {
    if(generarTirada){
        ...
        var numeroFichas = parseInt(document.getElementById("fichas").innerHTML);
        document.getElementById("fichas").innerHTML = (numeroFichas + ganancia);
        document.getElementById("fichas1").innerHTML = (numeroFichas + ganancia);
        llamada(ganancia, numeroFichas);
        llamadaApuestas(ganancia);
        if(ganancia>0){
    ...
    if(!cellId.includes("celdaVacia")){
        ...
        if(numeroFichas>=cantidadASumar){
            ...
            if (p >= 1) {
                ...
                ficha.currentTime = 0;
                ficha.play();
                document.getElementById("fichas").innerHTML = (numeroFichas - cantidadASumar);
                
                document.getElementById("fichas1").innerHTML = (numeroFichas - cantidadASumar);
                
                llamada(-cantidadASumar, numeroFichas)
...
function acceptRetirar() {
    ...
    if(!isNaN(cant) &&  cant<=numeroFichas && cant != ""){
        document.getElementById("fichas").innerHTML = (parseInt(numeroFichas) - parseInt(cant));
        document.getElementById("fichas1").innerHTML = (parseInt(numeroFichas) - parseInt(cant));
        llamada(-parseInt(cant), parseInt(numeroFichas));
        closeModalRetirar();
...
function llamada(fichas, fichasActuales) {
    $.ajax({
        type: "POST",
        url: "/llamadaFichasRuleta",
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ fichas: fichas }),
        success: function (data) {
			document.getElementById("fichas").innerHTML = parseInt(fichas) + parseInt(fichasActuales);
        },
...
'use strict'
function llamadaApuestas(ganado){
	var apostado = 0;
	var parrafo = document.getElementById("bet");
    var texto = parrafo.textContent;
    var numero = texto.split(":")[1].trim();
	...
	var fechaYHora = año + '-' + mes + '-' + dia + ' ' + horas + ':' + minutos + ':' + segundos;
